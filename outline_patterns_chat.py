...
def _path_to_polyline(path, bezier_samples=64):
    """Convert one complete PDF drawing path to one continuous polyline."""
    pts = []
    items = path.get("items", [])
    ...
    for item in items:
        ...
        if op == "l":
            ...
            p0, p1, p2, p3 = item[1], item[2], item[3], item[4]
            ts = np.linspace(0.0, 1.0, bezier_samples)
            for t in ts:
                x = ((1-t)**3*p0.x + 3*(1-t)**2*t*p1.x +
                     3*(1-t)*t**2*p2.x + t**3*p3.x)
                y = ((1-t)**3*p0.y + 3*(1-t)**2*t*p1.y +
                     3*(1-t)*t**2*p2.y + t**3*p3.y)
                add_point(fitz.Point(x, y))
        elif op == "re":
            r = item[1]
            add_point(fitz.Point(r.x0, r.y0))
            add_point(fitz.Point(r.x1, r.y0))
            add_point(fitz.Point(r.x1, r.y1))
            add_point(fitz.Point(r.x0, r.y1))
            add_point(fitz.Point(r.x0, r.y0))
        elif op == "qu":
            q = item[1]
            add_point(q.ul); add_point(q.ur); add_point(q.lr); add_point(q.ll); add_point(q.ul)

    return pts


def _extend_polyline_ends(points, extension=10.0):
    """Extend only the two ends of a complete polyline by a fixed distance."""
    pts = list(points)
    if len(pts) < 2 or extension <= 0:
        return pts
    ...
    def extend(p0, p1, amount):
        ...
        if L < 1e-9:
            return p0
        return (p0[0] - dx / L * amount,
                p0[1] - dy / L * amount)

    def extend_end(p0, p1, amount):
        dx = p1[0] - p0[0]
        dy = p1[1] - p0[1]
        L = math.hypot(dx, dy)
        if L < 1e-9:
            return p1
        return (p1[0] + dx / L * amount,
                p1[1] + dy / L * amount)

    pts[0] = extend(pts[0], pts[1], extension)
    pts[-1] = extend_end(pts[-2], pts[-1], extension)
    return pts


def _path_is_closed(path, points):
    if path.get("closePath"):
        return True
    return len(points) >= 3 and dist(points[0], points[-1]) <= 1.0
...
def detect_faces_for_piece(main_piece,
                           ...
                           snap_tol=2.5,
                           inside_tol=6.0,
                           cutter_extension=10.0):
    """
    Detect atomic faces inside an already detected main pattern piece.

    Important design rules:
      * The main piece geometry is the outer boundary. We never reconstruct
        it from raw PDF cutter segments.
      * Candidate cutters are COMPLETE PDF paths, not individual Bézier chords.
      * Paths having the same stroke style as the main outline are excluded
        from the cutter set. This prevents missed main-outline fragments from
        becoming false internal boundaries.
      * Only the two ends of an open cutter path are extended.
    """
    pts = main_piece.get("points")
    if not pts or len(pts) < 3:
        pts = _ordered_points_from_segments(main_piece.get("segments", []), dist)
    if len(pts) < 3:
        return []
    ...
    main_color = None
    if main_style and len(main_style) > 0:
        main_color = main_style[0]

    # --------------------------------------------------------------
    # Collect COMPLETE PDF paths as candidate cutters.
    # --------------------------------------------------------------
    candidates = []
    for path_id, path in all_paths_with_id:
        if path_id in main_path_ids:
            continue

        color = _path_color(path)
        if main_color is not None and color == main_color:
            # Same style as the outer outline: never treat this as a cutter.
            continue

        points = _path_to_polyline(path)
        if len(points) < 2:
            continue
        ...
        # A path is relevant if it enters the piece or lies just outside its
        # boundary. The latter is needed for small endpoint gaps.
        if not (outer.intersects(line) or outer.distance(line) <= inside_tol):
            continue

        closed = _path_is_closed(path, points)
        if closed:
            cutter = line
        else:
            extended = _extend_polyline_ends(points, cutter_extension)
            cutter = LineString(extended)
    ...
    print(f"  → {len(candidates)} candidate internal paths for this piece")
    if not candidates:
    ...
    # --------------------------------------------------------------
    # Build one line arrangement: outer boundary + complete cutters.
    # --------------------------------------------------------------
    boundary = LineString(list(outer.exterior.coords))
    all_lines = [boundary] + [c["line"] for c in candidates]

    try:
        noded = unary_union(MultiLineString(all_lines))
        raw_faces = list(polygonize(noded))
    except Exception as exc:
        print(f"  ! polygonization failed: {exc}")
        return []
...
def detect_all_faces(pieces, page, **kwargs):
    """Run face detection using the same complete PDF paths used by the page."""
    all_paths_with_id = list(enumerate(page.get_drawings()))
    print(f"Total PDF paths on page: {len(all_paths_with_id)}")

    out = []
    for i, piece in enumerate(pieces):
        faces = detect_faces_for_piece(piece, all_paths_with_id, **kwargs)
        out.append((i, faces))
